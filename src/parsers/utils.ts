...
export function parseOrderSize(sizeStr: string, rawInput: string): ParsedSize {
  ...
  if (cleaned.startsWith('$')) {
    ...
    if (isNaN(value) || value < 0) {
      throw new InvalidSizeFormatError(rawInput, sizeStr, 'positive dollar amount like $100 or $2.50');
    }
...
export function parseFillSize(sizeStr: string, rawInput: string): ParsedSize {
  ...
  if (cleaned.startsWith('$')) {
    const value = parseFloat(cleaned.substring(1));
    if (isNaN(value) || value < 0) {
      throw new InvalidSizeFormatError(rawInput, sizeStr, 'positive dollar amount like $100 or $2.00');
    }
    return { value, format: 'dollar' };
  }
  ...
  const value = parseFloat(cleaned);
  if (isNaN(value) || value < 0) {
    throw new InvalidSizeFormatError(rawInput, sizeStr, 'positive decimal number like 2.0 (=$2000) or 0.563 (=$563)');
  }
...
export function parsePeriod(periodStr: string, rawInput: string): Period {
  ...
  if (cleaned === 'f5' || cleaned === 'h1' || cleaned === '1h' || 
      cleaned === 'first half' || cleaned === '1st half' ||
      cleaned === 'first h' || cleaned === '1st h' ||
      cleaned === 'first five' || cleaned === '1st five' ||
      cleaned === 'first 5' || cleaned === '1st 5') {
    return { PeriodTypeCode: 'H', PeriodNumber: 1 };
  }
  
  // Second half patterns: h2, 2h, second half, 2nd half
  if (cleaned === 'h2' || cleaned === '2h' ||
      cleaned === 'second half' || cleaned === '2nd half' ||
      cleaned === 'second h' || cleaned === '2nd h') {
  ...
  if (inningMatch) {
    ...
    if (iNum >= 1 && iNum <= 15) { // Reasonable max for baseball
  ...
  if (periodMatch) {
    ...
    if (pNum >= 1 && pNum <= 4) { // 1-3 regulation + OT
...
export function parseGameNumber(gameStr: string, rawInput: string): number {
  ...
  if (gameNum < 1 || gameNum > 10) { // Reasonable bounds
    throw new InvalidGameNumberError(rawInput, gameStr);
  }
...
export function validatePropFormat(propText: string, hasLine: boolean, rawInput: string): void {
  ...
  if (propInfo.category === 'PropOU' && !hasLine) {
    throw new InvalidContractTypeError(rawInput, 
